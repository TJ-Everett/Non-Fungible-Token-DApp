...
// Import ABI + Config
import EthNinjas from '../abis/EthNinjas.json'
import config from '../config.json'

function App() {
	const [web3, setWeb3] = useState(null)
	const [ethNinjas, setEthNinjas] = useState(null)
	...
	const loadBlockchainData = async (_web3, _account, _networkId) => {
		...
		try {
			const ethNinjas = new _web3.eth.Contract(EthNinjas.abi, EthNinjas.networks[_networkId].address)
			setEthNinjas(ethNinjas)

			const maxSupply = await ethNinjas.methods.maxSupply().call()
			const totalSupply = await ethNinjas.methods.totalSupply().call()
			setSupplyAvailable(maxSupply - totalSupply)

			const allowMintingAfter = await ethNinjas.methods.allowMintingAfter().call()
			const timeDeployed = await ethNinjas.methods.timeDeployed().call()
			setRevealTime((Number(timeDeployed) + Number(allowMintingAfter)).toString() + '000')

			if (_account) {
				const ownerOf = await ethNinjas.methods.walletOfOwner(_account).call()
				setOwnerOf(ownerOf)
				console.log(ownerOf)
			} else {
				setOwnerOf([])
	...
	const mintNFTHandler = async () => {
		...
		// Mint NFT
		if (ethNinjas && account) {
			setIsMinting(true)
			setIsError(false)

			await ethNinjas.methods.mint(1).send({ from: account, value: 0 })
				.on('confirmation', async () => {
					const maxSupply = await ethNinjas.methods.maxSupply().call()
					const totalSupply = await ethNinjas.methods.totalSupply().call()
					setSupplyAvailable(maxSupply - totalSupply)

					const ownerOf = await ethNinjas.methods.walletOfOwner(account).call()
					setOwnerOf(ownerOf)
				})
				.on('error', (error) => {
					window.alert(error)
	...
	const cycleImages = async () => {
		const getRandomNumber = () => {
			const counter = (Math.floor(Math.random() * 531)) + 1
			setCounter(counter)
		}

		if (!isCycling) { setInterval(getRandomNumber, 10000) }
		setIsCycling(true)
	}
			...
			<main>
				<section id='welcome' className='welcome'>
					...
					<Row className='flex m-3'>
						...
						<Col md={5} lg={4} xl={5} xxl={4}>
							...
							<p className='text'>
								Check out my NFT Collection. This is a set of 531 hand drawn NFT Ninjas. Mint one below. 
							</p>
				...
				<section id='about' className='about'>
					...
					<Row className='flex m-3'>
						...
						<Col md={5} lg={4} xl={5} xxl={4} className='text-center'>
							<img src={NinjaShowcase} width={300} height={310} alt="NinjaShowcase" className='NinjaShowcase' />
						</Col>
						<Col md={5} lg={4} xl={5} xxl={4}>
							{isError ? (
								<p>{message}</p>
								...
								<div>
									...
									<ul>
										<li>531 generated hand drawn images using an art generator</li>
									...
									{ownerOf.length > 0 &&
										<p><small>View your NFT on
											<a
												href={`${openseaURL}/assets/${ethNinjas._address}/${ownerOf[0]}`}
												target='_blank'
					...
					<Row style={{ marginTop: "100px" }}>
						<Col>
							{ethNinjas &&
								<a
									href={`${explorerURL}/address/${ethNinjas._address}`}
									target='_blank'
